...
\begin{abstract}
Payment channel networks is a promising proposal to deal with the issues of scaling the current Bitcoin system.
The Lightning Network is one of the most prominent of these types of networks. It deals with the scaling problems by allowing users to do transactions outside the Bitcoin network. The Lightning Network becomes a second layer on top of the Bitcoin network and can therefore scale independently. It has many secure mechanisms to allow for payments to be done without any trust or central authority similarly as Bitcoin. In addition to solving the scaling issue the Lightning Network can allow for faster payments, better privacy, cross-chain transfers and micropayments becomes feasible.
\end{abstract}
